...
A = ["q_bp", "T_A", "T_B", "T_C"];       # algebraic variables
...
### Defining model
model = create_model(linear_solver = "ma57", print_level = 0);
...
set_upper_bound.(u["q_whb",:], q_whb_ub);
...
# logic constraint (and additional necessary variables)
@variable(model, aux[1:2, i in 1:nfe, j in 1:ncp] >= 0, start = 0);
@constraint(model, aux_con[i in 1:nfe, j in 1:ncp], z["T_B",i,j] - T_dh_maxSup == aux[1,i,j] - aux[2,i,j]);
@NLexpression(model, Tbsl[i in 1:nfe, j in 1:ncp], T_dh_maxSup - (z["T_B",i,j] - aux[1,i,j]));                #how much T_B is lower than maximum temperature supply
@NLconstraint(model, [i in 1:nfe, j in 1:ncp], Tbsl[i,j] >= 0);
@NLexpression(model, qwsl[i in 1:nfe], q_whb_ub - u["q_whb",i]);                                              #slack to maximum capacity of WHB
@NLexpression(model, Tpsl[i in 1:nfe, j in 1:ncp], x["T_phb",i,j] - T_dh_minSup);                             #slack to minimum temperature supply              
@NLconstraint(model, [i in 1:nfe], qwsl[i] >= 0);
@NLconstraint(model, [i in 1:nfe, j in 1:ncp], Tpsl[i,j] >= 0);

# Solving by simply adding a penalization term for the complementarity constraint
@NLobjective(model, Min, sum(1e2*u["Q_phb",i]^2 for i in 1:nfe)                                     #goal: minimize fuel consumption
                         + sum(1e4*u["q_A",i]*u["q_B",i] for i in 1:nfe)                            #complementarity penalty for qA and qB
                         + sum(1e4*aux[1,i,j]*aux[2,i,j] for i in 1:nfe for j in 1:ncp)             #complementarity penalty for aux variables
                         + sum(1e4*Tbsl[i,j]*z["q_bp",i,j] for i in 1:nfe for j in 1:ncp)           #complementarity penalty for using the bypass
                         + sum(1e4*qwsl[i]*Tpsl[i,j] for i in 1:nfe for j in 1:ncp));               #complementarity penalty for storing energy
